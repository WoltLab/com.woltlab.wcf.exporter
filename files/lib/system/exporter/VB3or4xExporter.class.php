...
{
    private const FORUMOPTIONS_ACTIVE = 1;

    private const FORUMOPTIONS_ALLOWPOSTING = 2;

    private const FORUMOPTIONS_CANCONTAINTHREADS = 4;

    private const FORUMOPTIONS_MODERATENEWPOST = 8;

    private const FORUMOPTIONS_MODERATENEWTHREAD = 16;

    private const FORUMOPTIONS_MODERATEATTACH = 32;

    private const FORUMOPTIONS_ALLOWBBCODE = 64;

    private const FORUMOPTIONS_ALLOWIMAGES = 128;

    private const FORUMOPTIONS_ALLOWHTML = 256;

    private const FORUMOPTIONS_ALLOWSMILIES = 512;

    private const FORUMOPTIONS_ALLOWICONS = 1024;

    private const FORUMOPTIONS_ALLOWRATINGS = 2048;

    private const FORUMOPTIONS_COUNTPOSTS = 4096;

    private const FORUMOPTIONS_CANHAVEPASSWORD = 8192;

    private const FORUMOPTIONS_INDEXPOSTS = 16384;

    private const FORUMOPTIONS_STYLEOVERRIDE = 32768;

    private const FORUMOPTIONS_SHOWONFORUMJUMP = 65536;

    private const FORUMOPTIONS_PREFIXREQUIRED = 131072;

    private const FORUMPERMISSIONS_CANVIEW = 1;

    private const FORUMPERMISSIONS_CANVIEWTHREADS = 524288;

    private const FORUMPERMISSIONS_CANVIEWOTHERS = 2;

    private const FORUMPERMISSIONS_CANSEARCH = 4;

    private const FORUMPERMISSIONS_CANEMAIL = 8;

    private const FORUMPERMISSIONS_CANPOSTNEW = 16;

    private const FORUMPERMISSIONS_CANREPLYOWN = 32;

    private const FORUMPERMISSIONS_CANREPLYOTHERS = 64;

    private const FORUMPERMISSIONS_CANEDITPOST = 128;

    private const FORUMPERMISSIONS_CANDELETEPOST = 256;

    private const FORUMPERMISSIONS_CANDELETETHREAD = 512;

    private const FORUMPERMISSIONS_CANOPENCLOSE = 1024;

    private const FORUMPERMISSIONS_CANMOVE = 2048;

    private const FORUMPERMISSIONS_CANGETATTACHMENT = 4096;

    private const FORUMPERMISSIONS_CANPOSTATTACHMENT = 8192;

    private const FORUMPERMISSIONS_CANPOSTPOLL = 16384;

    private const FORUMPERMISSIONS_CANVOTE = 32768;

    private const FORUMPERMISSIONS_CANTHREADRATE = 65536;

    private const FORUMPERMISSIONS_FOLLOWFORUMMODERATION = 131072;

    private const FORUMPERMISSIONS_CANSEEDELNOTICE = 262144;

    private const FORUMPERMISSIONS_CANTAGOWN = 1048576;

    private const FORUMPERMISSIONS_CANTAGOTHERS = 2097152;

    private const FORUMPERMISSIONS_CANDELETETAGOWN = 4194304;

    private const FORUMPERMISSIONS_CANSEETHUMBNAILS = 8388608;
    ...
    private const GALLERY_DATABASE = 'db';

    private const GALLERY_FILESYSTEM = 'fs';

    private const GALLERY_FILESYSTEM_DIRECT_THUMBS = 'fs_directthumb';
    ...
    public function getQueue()
    {
        ...
        if (\in_array('com.woltlab.calendar.event', $this->selectedData)) {
            if (\in_array('com.woltlab.calendar.category', $this->selectedData)) {
                $queue[] = 'com.woltlab.calendar.category';
            }
    ...
    public function exportUserRanks($offset, $limit)
    {
        $sql = "(
                ...
                UNION
                (
                    SELECT  ('g-' || usergroupid) AS usertitleid, usergroupid AS groupID,
                            0 AS minposts, usertitle AS title
                    FROM    " . $this->databasePrefix . "usergroup
                    WHERE   usergroupid NOT IN(?, ?)
                        AND usertitle <> ?
                )
    ...
    public function exportConversationMessages($offset, $limit)
    {
        $sql = "SELECT      pmtext.*,
                            (
                                " . $this->database->handleLimitParameter("
                                    SELECT  IF(pm.parentpmid = 0, pm.pmid, pm.parentpmid)
                                    FROM    " . $this->databasePrefix . "pm pm
    ...
    protected function exportBoardsRecursively($parentID = -1)
    {
        ...
        $getDaysPrune = static function ($value) {
            if ($value == -1) {
                return 1000;
            }
    ...
    public function exportThreads($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            $additionalData = [];
            if ($row['prefixid']) {
                $additionalData['labels'] = [$row['prefixid']];
            }
            if ($row['taglist'] !== null) {
    ...
    {
        $sql = "SELECT      post.*,
                            postedithistory.dateline AS lastEditTime,
                            postedithistory.username AS editor,
                            postedithistory.userid AS editorID,
    ...
    public function exportWatchedThreads($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()
                ...
                ->import($row['subscribethreadid'], $data);
        }
    }
    ...
    public function exportGalleryImages($offset, $limit)
    {
        try {
            ...
            $sql = "SELECT      attachment.*, attachment.contentid AS albumid, filedata.filedata, filedata.extension,
                    ...
                    WHERE       attachment.contenttypeid = (
                                    SELECT  contenttypeid
                                    FROM    " . $this->databasePrefix . "contenttype contenttype
        ...
        while ($row = $statement->fetchArray()) {
            try {
                if ($vB === 4) {
                    switch ($this->readOption('attachfile')) {
                        ...
                        case self::ATTACHFILE_FILESYSTEM:
                            ...
                            $file .= $row['userid'] . '/' . ($row['filedataid'] ?? $row['attachmentid']) . '.attach';
                            break;
                        case self::ATTACHFILE_FILESYSTEM_SUBFOLDER:
                            ...
                            $file = FileUtil::addTrailingSlash($file);
                            $file .= \implode('/', \str_split($row['userid'])) . '/' . ($row['filedataid'] ?? $row['attachmentid']) . '.attach';
                            break;
                    }
                } else {
                ...
                $data = [
                    ...
                    'description' => '',
                    'filename' => StringUtil::decodeHTML($row['filename'] ?? ''),
    ...
    public function exportGalleryComments($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            $data = [
                'objectID' => $row['pictureid'] ?? $row['filedataid'],
    ...
    public function exportCalendarCategories($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()
                ->getImporter('com.woltlab.calendar.category')
                ->import($row['calendarid'], $data);
        }
    }
        ...
        while ($row = $statement->fetchArray()) {
            ...
            switch ($row['recurring']) {
                ...
                case 3:
                    ...
                    [$interval, $days] = \explode('|', $row['recuroption']);
                ...
                case 6:
                    ...
                    [$month, $day] = \explode('|', $row['recuroption']);
                ...
                case 7:
                    ...
                    [$offset, $day, $month] = \explode('|', $row['recuroption']);
                    $eventDateData['repeatYearlyByMonth'] = $month;
