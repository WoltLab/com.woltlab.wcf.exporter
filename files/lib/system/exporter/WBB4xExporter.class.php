...
final class WBB4xExporter extends AbstractExporter
{
    ...
    public function getQueue()
    {
        ...
        if ($this->getPackageVersion('com.woltlab.gallery')) {
            if (\in_array('com.woltlab.gallery.image', $this->selectedData)) {
                ...
                if (\in_array('com.woltlab.gallery.image.marker', $this->selectedData)) {
                    $queue[] = 'com.woltlab.gallery.image.marker';
                }
            }
        }
        ...
        if ($this->getPackageVersion('com.woltlab.calendar')) {
            if (\in_array('com.woltlab.calendar.event', $this->selectedData)) {
                ...
                if (\in_array('com.woltlab.calendar.event.attachment', $this->selectedData)) {
                    $queue[] = 'com.woltlab.calendar.event.attachment';
                }
                ...
                if (\in_array('com.woltlab.calendar.event.like', $this->selectedData)) {
                    $queue[] = 'com.woltlab.calendar.event.like';
                }
                if (\in_array('com.woltlab.calendar.event.date.participation', $this->selectedData)) {
                    $queue[] = 'com.woltlab.calendar.event.date.participation';
                }
            }
        }
        ...
        if (\version_compare($this->getPackageVersion('com.woltlab.wcf'), '3.0.0 Alpha 1', '>=')) {
            ...
            if (\in_array('com.woltlab.wcf.media', $this->selectedData)) {
                if (\in_array('com.woltlab.wcf.media.category', $this->selectedData)) {
                    $queue[] = 'com.woltlab.wcf.media.category';
                }
    ...
    public function exportUserGroups($offset, $limit)
    {
        ...
        foreach ($groups as $groupID => $groupData) {
            $i18nData = [];
            if (isset($i18nValues[$groupData['groupName']])) {
                $i18nData['groupName'] = $i18nValues[$groupData['groupName']];
            }
            if (isset($i18nValues[$groupData['groupDescription']])) {
                $i18nData['groupName'] = $i18nValues[$groupData['groupDescription']];
            }
    ...
    public function exportUsers($offset, $limit)
    {
        ...
        $sql = "SELECT      user_option_value.*, user_table.*, avatars.*,
                ...
                LEFT JOIN   wcf" . $this->dbNo . "_user_avatar avatars
    ...
    {
        $this->exportCommentResponses(
            'com.woltlab.wcf.user.profileComment',
            'com.woltlab.wcf.user.comment.response',
            $offset,
            $limit
        );
    }
    ...
    public function exportConversations($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()
                ...
                ->import($row['conversationID'], $data);
        }
    }
        ...
        if (!empty($i18nValues)) {
            foreach ($this->boardCache as &$boards) {
                foreach ($boards as &$board) {
                    $board['i18n'] = [];
                    if (isset($i18nValues[$board['title']])) {
                        $board['i18n']['title'] = $i18nValues[$board['title']];
                    }
                    if (isset($i18nValues[$board['description']])) {
                        $board['i18n']['description'] = $i18nValues[$board['description']];
                    }
                }
                unset($board);
            }
    ...
    public function exportThreads($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            if (isset($labels[$row['threadID']])) {
                $additionalData['labels'] = $labels[$row['threadID']];
            }
            if (isset($tags[$row['threadID']])) {
                $additionalData['tags'] = $tags[$row['threadID']];
            }
    ...
    public function exportLabels($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            if (!isset($boardIDs[$row['groupID']])) {
                $boardIDs[$row['groupID']] = [];
            }
        ...
        if (!empty($labelGroups)) {
            ...
            foreach ($labelGroups as $labelGroup) {
                ...
                $additionalData = [
                    'objects' => [
                        $objectType->objectTypeID => !empty($boardIDs[$labelGroup['groupID']]) ? $boardIDs[$labelGroup['groupID']] : [],
                    ],
                ];
    ...
    public function exportSmileyCategories($offset, $limit)
    {
        $this->exportCategories(
            'com.woltlab.wcf.bbcode.smiley',
            'com.woltlab.wcf.smiley.category',
            $offset,
            $limit
        );
    }
    ...
    {
        if (
            \version_compare($this->getPackageVersion('com.woltlab.blog'), '2.1.0 Alpha 1', '>=')
    ...
    {
        $sourceVersion21 = \version_compare(
            $this->getPackageVersion('com.woltlab.blog'),
            '2.1.0 Alpha 1',
            '>='
        );
        ...
        while ($row = $statement->fetchArray()) {
            ...
            if (isset($tags[$row['entryID']])) {
                $additionalData['tags'] = $tags[$row['entryID']];
            }
            if (isset($categories[$row['entryID']])) {
    ...
    public function exportBlogCommentResponses($offset, $limit)
    {
        $this->exportCommentResponses(
            'com.woltlab.blog.entryComment',
            'com.woltlab.blog.entry.comment.response',
            $offset,
            $limit
        );
    }
    ...
    {
        $sourceVersion21 = \version_compare(
            $this->getPackageVersion('com.woltlab.gallery'),
            '2.1.0 Alpha 1',
            '>='
        );
        $destVersion21 = \version_compare(
    ...
    public function exportGalleryImages($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            if ($sourceVersion21 && $destVersion21) {
                $images[$row['imageID']] = \array_merge(
                    $images[$row['imageID']],
                    [
                        'enableHtml' => $row['enableHtml'],
    ...
    {
        if (
            \version_compare($this->getPackageVersion('com.woltlab.gallery'), '2.1.0 Alpha 1', '>=')
    ...
    public function exportGalleryImageMarkers($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()
                ->getImporter('com.woltlab.gallery.image.marker')
                ->import($row['markerID'], $data);
        }
    }
    ...
    {
        $this->exportComments(
            'com.woltlab.gallery.imageComment',
            'com.woltlab.gallery.image.comment',
            $offset,
            $limit
        );
    ...
    public function exportGalleryCommentResponses($offset, $limit)
    {
        $this->exportCommentResponses(
            'com.woltlab.gallery.imageComment',
            'com.woltlab.gallery.image.comment.response',
            $offset,
            $limit
        );
    }
    ...
    public function exportCalendarEvents($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            if (isset($tags[$row['eventID']])) {
                $additionalData['tags'] = $tags[$row['eventID']];
            }
            ...
            if (\version_compare($this->getPackageVersion('com.woltlab.calendar'), '3.0.0 Alpha 1', '<')) {
                // 2.x
                if (isset($categories[$row['eventID']])) {
                    $additionalData['categories'] = $categories[$row['eventID']];
                }
            } else {
                // 3.0+
    ...
    public function exportCalendarEventDateParticipation($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()
                ->getImporter('com.woltlab.calendar.event.date.participation')
                ->import($row['participationID'], $data);
        }
    }
    ...
    {
        $this->exportComments(
            'com.woltlab.calendar.eventDateComment',
            'com.woltlab.calendar.event.date.comment',
            $offset,
            $limit
        );
    ...
    public function exportCalendarEventDateCommentResponses($offset, $limit)
    {
        $this->exportCommentResponses(
            'com.woltlab.calendar.eventDateComment',
            'com.woltlab.calendar.event.date.comment.response',
            $offset,
            $limit
        );
    }
    ...
    {
        $this->exportLikes(
            'com.woltlab.calendar.likeableEvent',
            'com.woltlab.calendar.event.like',
            $offset,
            $limit
        );
    ...
    public function exportFilebaseFiles($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            $this->exportFilebaseFilesHelper(
                $row,
                $contents,
                $categories[$row['fileID']] ?? [$row['categoryID']]
            );
    ...
    {
        $this->exportComments(
            'com.woltlab.filebase.fileComment',
            'com.woltlab.filebase.file.comment',
            $offset,
            $limit
        );
    ...
    public function exportFilebaseFileCommentResponses($offset, $limit)
    {
        $this->exportCommentResponses(
            'com.woltlab.filebase.fileComment',
            'com.woltlab.filebase.file.comment.response',
            $offset,
            $limit
        );
    }
    ...
    {
        $this->exportLikes(
            'com.woltlab.filebase.likeableFile',
            'com.woltlab.filebase.file.like',
            $offset,
            $limit
        );
    ...
    {
        $this->exportLikes(
            'com.woltlab.filebase.likeableFileVersion',
            'com.woltlab.filebase.file.version.like',
            $offset,
            $limit
        );
    }
    ...
    {
        $this->exportAttachments(
            'com.woltlab.filebase.version',
            'com.woltlab.filebase.file.version.attachment',
            $offset,
            $limit
        );
    }
    ...
    public function exportPages($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            $data = [
                ...
                'lastUpdateTime' => $row['lastUpdateTime'],
                'cssClassName' => ($row['cssClassName'] ?? ''),
                'availableDuringOfflineMode' => ($row['availableDuringOfflineMode'] ?? 0),
                'allowSpidersToIndex' => ($row['allowSpidersToIndex'] ?? 1),
            ];
    ...
    public function exportArticleCommentResponses($offset, $limit)
    {
        $this->exportCommentResponses(
            'com.woltlab.wcf.articleComment',
            'com.woltlab.wcf.article.comment.response',
            $offset,
            $limit
        );
    }
    ...
    private function getTags($objectType, array $objectIDs)
    {
        ...
        $conditionBuilder->add(
            ...
            [$this->getObjectTypeID('com.woltlab.wcf.tagging.taggableObject', $objectType)]
        );
        $conditionBuilder->add(
            'tag_to_object.objectID IN (?)',
            [$objectIDs]
        );
    ...
    private function getLabels($objectType, array $objectIDs)
    {
        ...
        $conditionBuilder->add(
            'objectTypeID = ?',
            [$this->getObjectTypeID('com.woltlab.wcf.label.object', $objectType)]
        );
        $conditionBuilder->add(
            'objectID IN (?)',
            [$objectIDs]
    ...
    private function getFilebaseDir()
    {
        ...
        $row = $statement->fetchArray();
        if ($row !== false) {
            return FileUtil::getRealPath($this->fileSystemPath . $row['packageDir']);
        }
