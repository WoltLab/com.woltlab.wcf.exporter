...
final class PhpBB31xExporter extends AbstractExporter
{
    ...
    private const AVATAR_TYPE_GALLERY = 'avatar.driver.local';

    private const AVATAR_TYPE_REMOTE = 'avatar.driver.remote';

    private const AVATAR_TYPE_UPLOADED = 'avatar.driver.upload';

    private const AVATAR_TYPE_NO_AVATAR = '';
    ...
    private const ITEM_UNAPPROVED = 0;

    private const ITEM_APPROVED = 1;

    private const ITEM_DELETED = 2;

    private const ITEM_REAPPROVE = 3;
    ...
    public function validateFileAccess()
    {
        if (
            ...
            if (
                ...
                || !@\file_exists($this->fileSystemPath . 'phpbb/error_collector.php')
            ) {
                return false;
            }
        }
    ...
    public function exportConversationUsers($offset, $limit)
    {
        ...
        while ($row = $statement->fetchArray()) {
            ...
            ImportHandler::getInstance()->getImporter('com.woltlab.wcf.conversation.user')
                ->import(
                    0,
                    $data,
                    [
    ...
    protected static function fixBBCodes($text, $uid)
    {
        ...
        // fix size bbcode
        $text = \preg_replace_callback('~(?<=\[size=)\d+(?=\])~', static function ($matches) {
            $wbbSize = 24;
            if ($matches[0] <= 50) {
                $wbbSize = 8;
            } elseif ($matches[0] <= 85) {
        ...
        $text = \preg_replace('~\[attachment=(\d+)\].*?\[/attachment\]~', '', $text);
